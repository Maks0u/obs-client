...
export class Scene {
     ...
     */
    async init() {
        this.items = new Map((await this.getItemList()).map(i => [i.sourceUuid, i]));
        this.inited = true;
    }
    ...
    async getItemList() {
        ...
        return (await this.obs.ws.call('GetSceneItemList', { sceneUuid })).sceneItems.map(
            i => new SceneItem(this.obs, sceneUuid, i)
        );
    }
